...
const CONFIG = {
  // URL do Blob Storage com Token de LEITURA (somente leitura — seguro para o portal)
  API_URL: 'https://lminformatica.blob.core.windows.net/portal-data/os.json?sv=2025-11-05&ss=b&srt=o&sp=rtfx&se=2027-04-05T08:56:10Z&st=2026-05-05T00:41:10Z&spr=https&sig=YFpszeNU9GhD4O02h32%2FoHPDGeVOAcOSKFO6wMfOFwk%3D',
  ATUALIZAR: 300, // 5 minutos
...
async function loadOS() {
  try {
    // Busca o JSON do Azure Blob Storage com anti-cache
    const url = `${CONFIG.API_URL}&t=${Date.now()}`;
    const res = await fetch(url);
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    const data = await res.json();

    // Suporta formato { gerado_em, os: [...] } ou array direto
    ...
    // Fallback para dados demo se falhar
    if (allOS.length === 0) {
      allOS = DEMO_OS;
      const badge = document.querySelector('.systec-badge');
